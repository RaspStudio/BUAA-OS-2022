...
page_fault_handler(struct Trapframe *tf)
{
    ...
    // Save Current TrapFrame To Temp Var
    bcopy(tf, &PgTrapFrame, sizeof(struct Trapframe));

    // If Already in XSTACK, push; else change to XSTACK (such as first call: STK -> XTSK)
...
/*---------- MY HANDLER OF PANIC ----------*/
void do_reserved_out(int code) {
    panic("UNINPLEMENTED EXC[%d]!", code);
}
