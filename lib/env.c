...
/* My Comment:
 *  ENVID: asid[16-11] + 1[10] + index[9-0]
 */
...
env_setup_vm(struct Env *e)
{
    ...
    /* When Env try to access UVPT, will get PADDR of env_pgdir. (User Virtual Page Table / User Page Directory) */
...
static int load_page(u_long va, struct Env *env, struct Page **retdst) {
    struct Page *ret = page_lookup(env->env_pgdir, va, NULL);

    int r;
    if (ret == NULL) {
        /* No Existing Page, Alloc New Page */
        if ((r = page_alloc(&ret)) != 0) {
            *retdst = NULL;
            return r;
        } 
        if ((r = page_insert(env->env_pgdir, ret, va, PTE_R)) != 0) {
            *retdst = NULL;
            return r;
        }
    }
...
static void load_binary(u_long dstva, u_long srcva, u_int32_t size) {
    u_long offset = ROUND(dstva, 4) - dstva;

    if ((void*)srcva == NULL) {
        if (offset) bzero((void*)dstva, offset);
        bzero((void*)(dstva + offset), size - offset);
    } else {
        bcopy((void*)srcva, (void*)dstva, size);
    }
}

static int load_data(u_long dstva, u_char *srcva, u_int32_t datasize, struct Env *env) {
    u_long i, r;
    ...
    for (i = 0; i < datasize; i += size) {
        struct Page *ppage;
        /* Get Dest Page */
        if ((r = load_page(dstva + i, env, &ppage)) != 0) {
            return r;
        }
        /* Write Data */
        size = MIN((datasize - i), (BY2PG - offset));
        if (dbg) printf("Loading: [%s: 0x%08x->] (%d - %d)/%d\n", srcva == NULL ? "SEG" : "BIN", page2kva(ppage) + offset, i,  i + size, datasize);
        load_binary(page2kva(ppage) + offset,
                    srcva == NULL ? 0 : (u_long)srcva + i,
                    size);
        offset = 0;
    }
    if (dbg) printf("Loading: [%s] Load End \n", srcva == NULL ? "SEG" : "BIN");
    return 0;
}
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    if ((r = load_data(va, bin, bin_size, env)) != 0) {
        return r;
    }
    if ((r = load_data(va + bin_size, NULL, sgsize - bin_size, env)) != 0) {
        return r;
    }
...
{
    if(dbg) printf("Called load_icode\n");
    /* Hint:
...
{
    if(dbg) printf("Called env_create_priority\n");
    struct Env *e;
...
{
    if(dbg) printf("Called env_create\n");    
...
env_run(struct Env *e)
{
    ...
    if (dbg) printf("[ENV_RUN] Called...\n");
    if (curenv != NULL) {
        struct Trapframe *cur_tf = (struct Trapframe*)(TIMESTACK - sizeof(struct Trapframe));
        curenv->env_tf = *cur_tf;
        curenv->env_tf.pc = curenv->env_tf.cp0_epc;
        if (dbg) printf("[ENV_RUN] Saved Current Env...\n");
    }
    if (dbg) printf("[ENV_RUN] Prepare New Env...\n");
    ...
    curenv = e;
    curenv->env_runs++;
    if (dbg) printf("[ENV_RUN] Now CURENV = ENV[%d]", curenv->env_id);
    /* Step 3: Use lcontext() to switch to its address space. */
    lcontext((u_int)curenv->env_pgdir);
    if (dbg) printf("[ENV_RUN] Switched VA Space for New Env...\n");
