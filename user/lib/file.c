...
int open(const char *path, int mode) {
	...
	/*----- Alloc a new FD struct -----*/
	struct Fd *fd;
	if ((r = fd_alloc(&fd)) != 0) {
		/* Failed To Alloc File Descriptor */
		return r;
	}

	/*----- Call FS Server to Open File -----*/
	if ((r = fsipc_open(path, mode, fd)) != 0) {
		/* FS Server Failed To Serve */
		return r;
	}

	/*----- Get Pointer (FS Serv use fd as Filefd*) -----*/
	struct Filefd *ffd = (struct Filefd*)fd;

	/*----- Map All File Data Area -----*/
	u_int va = fd2data(fd);
	u_int size = ffd->f_file.f_size;
	u_int i;

	for (i = 0; i < size; i+=BY2PG) {
		if ((r = fsipc_map(ffd->f_fileid, i, va + i)) != 0) {
			/* FS Serv Failed To Map */
			return r;
		}
	}
