...
serve_open(u_int envid, struct Fsreq_open *rq)
{
	...
	//	user_panic("file_open failed: %d, invalid path: %s", r, path);
		if ((r == -E_NOT_FOUND) && (rq->req_omode & O_CREAT)) {
			if ((r = file_create((char*)path, &f)) < 0) {
				writef(B_RED "FileSystem" C_RESET ": Open \'%s\' Failed.\n", rq->req_path);
				ipc_send(envid, r, 0, 0);
				return;
			}
			createFlag = 1;
	...
	OUT writef(B_MAGENTA "FileSystem" C_RESET "[%d]: %s \'%s\' Success.\n", env->env_id, createFlag ? "Create" : "Open", rq->req_path);
}
...
umain(void)
{
	...
	fs_init();
	fs_test();
	//writef("\033[34m[FILE SYSTEM]\tMOS File System Initialized!\033[0m\n");
	serve();
}
